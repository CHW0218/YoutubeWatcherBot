...
def fetch_video(google_api_key,youtube_playlist_id, page_token=None):
    payload = fetch_video_page(google_api_key,youtube_playlist_id,page_token)
    yield from payload['items']
    next_page_token = payload.get('nextPageToken')
    if next_page_token is not None:
        yield from fetch_video(google_api_key,youtube_playlist_id, next_page_token)
...
def main():
    logging.info('Starting youtube_watcher')
    ...
    for video_item in fetch_playlist_items(google_api_key,youtube_playlist_id):
        ...
        for video in fetch_video(google_api_key,video_id):
            logging.info("Video: %s", pformat(summarize_video(video)))
